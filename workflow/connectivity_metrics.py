...
def create_connectivity_matrix(output_dir,cifti_file, 
                          parcel_file,parcel_name,
                          method):
    ...
    print('\t-Cifti file: ' + cifti_file)
    print('\t-Parcel file: ' + parcel_file)
    print('\t-Network matrix method/type: ' + str(method))
    
    # create output folder if it does not exist
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    
    #now read parcel files and generate parcel labels
    try:
    ...
    parcel_labels = [str(r) for r in parcel_labels]
    ...
    os.system("/opt/workbench/bin_rh_linux64/wb_command -cifti-parcellate %s %s %s %s" 
              % (cifti_file, 
                 parcel_file, 
                 "COLUMN", 
                 os.path.join(output_dir,cifti_prefix) + "_"+parcel_name + ".ptseries.nii"))
    cifti_file = os.path.join(output_dir,cifti_prefix) + "_"+parcel_name +".ptseries.nii"
   
    # check that parcellation worked
    try:
        read_cifti = open(cifti_file)
        read_cifti.close()
    except IOError:
    ...
    # now generate connectivity matrix based on method
    ...
    def create_file_output(self,ICAstring,text_output_dir,level,data_output_format):
        ...
        print('\t-Network matrix output method/type: %s' %str(method))
        ...
        if data_output_format == "NUMPY" or data_output_format == "numpy":
            ...
            output_text_file = os.path.join(text_output_dir,"_".join(self.fmriname.split('_')[2:])+"_"+self.parcel_name+ICAstring+'_level'+ str(level)+'_method'+method+".csv")
